...
int picovoice_main(int argc, char **argv) {
  ...
  if (!(access_key && library_path && model_path && (optind < argc))) {
    fprintf(stderr, "usage: -a ACCESS_KEY -m MODEL_PATH -l LIBRARY_PATH [-d] "
                    "[-v] audio_path0 audio_path1 ...\n");
    exit(1);
  }
  ...
  pv_status_t (*pv_leopard_init_func)(const char *, const char *, bool,
                                      pv_leopard_t **) =
  ...
  void (*pv_leopard_delete_func)(pv_leopard_t *) =
      load_symbol(dl_handle, "pv_leopard_delete");
  ...
  pv_status_t (*pv_leopard_process_file_func)(
      pv_leopard_t *, const char *, char **, int32_t *, pv_word_t **) =
  ...
  pv_status_t status = pv_leopard_init_func(
      access_key, model_path, enable_automatic_punctuation, &leopard);
    ...
    if (show_metadata) {
      for (int32_t j = 0; j < num_words; j++) {
        fprintf(stdout,
                ...
                words[j].word, words[j].start_sec, words[j].end_sec,
                words[j].confidence);
      }
      printf("\n");
    }
