 ...
 * @param enable_automatic_punctuation Set to `true` to enable automatic
 ...
 * `PV_STATUS_INVALID_ARGUMENT`, `PV_STATUS_RUNTIME_ERROR`,
 ...
 * failure.
 */
PV_API pv_status_t pv_leopard_init(const char *access_key,
...
/**
 * Processes a given audio data and returns its transcription. The caller is
 * responsible for freeing the transcription buffer.
 *
 * @param object Leopard object.
 * @param pcm Audio data. The audio needs to have a sample rate equal to
 * `pv_sample_rate()` and be 16-bit linearly-encoded. This function operates on
 ...
 * failure
 */
PV_API pv_status_t pv_leopard_process(pv_leopard_t *object, const int16_t *pcm,
                                      int32_t num_samples, char **transcript,
                                      int32_t *num_words, pv_word_t **words);

/**
 * Processes a given audio file and returns its transcription. The caller is
 * responsible for freeing the transcription buffer.
 *
 * @param object Leopard object.
 * @param audio_path Absolute path to the audio file. The file needs to have a
 * sample rate equal to or greater than `pv_sample_rate()`. The supported
 * formats are: `3gp (AMR)`, `FLAC`, `MP3`, `MP4/m4a (AAC)`, `Ogg`, `WAV`,
 * `WebM`. Files with stereo audio are mixed into a single mono channel and then
 * processed.
 * @param[out] transcript Inferred transcription.
 * @param[out] num_words Number of transcribed words
 * @param[out] words Transcribed words and their associated metadata.
 ...
 * failure
 */
PV_API pv_status_t pv_leopard_process_file(pv_leopard_t *object,
...
/**
 * Deletes transcript returned from `pv_leopard_process()` or
 * `pv_leopard_process_file()`
 *
 * @param transcript transcription string returned from `pv_leopard_process()`
 * or `pv_leopard_process_file()`
 */
PV_API void pv_leopard_transcript_delete(char *transcript);

/**
 * Deletes words returned from `pv_leopard_process()` or
 * `pv_leopard_process_file()`
 *
 * @param words transcribed words returned from `pv_leopard_process()` or
 * `pv_leopard_process_file()`
 */
