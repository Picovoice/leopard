...
class Utils {
    ...
    private static Path getResourceDirectory() throws RuntimeException {
        ...
        final URL resourceURL = Leopard.class.getProtectionDomain().getCodeSource().getLocation();
        Path resourcePath;
        ...
        if (resourcePath.toString().endsWith(".jar")) {
            try {
                ...
                throw new RuntimeException("Failed to extract resources from Leopard jar.");
            }
        }

        return resourcePath.resolve("leopard");
    }
    ...
    private static Path extractResources(Path jarPath) throws IOException {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (jarEntryName.startsWith("leopard")) {
    ...
    public static String getEnvironmentName() throws RuntimeException {
        ...
        if (os.contains("mac") || os.contains("darwin")) {
            ...
            throw new RuntimeException("Execution environment not supported. " +
                    "Leopard Java is supported on MacOS, Linux and Windows");
        }
    }
    ...
    private static String getCpuPart() throws RuntimeException {
        try {
            ...
            throw new RuntimeException("Leopard failed to get get CPU information.");
        }
    }

    public static String getPackagedModelPath() {
        return RESOURCE_DIRECTORY.resolve("lib/common/leopard_params.pv").toString();
    }

    public static String getPackagedLibraryPath() {
        switch (ENVIRONMENT_NAME) {
            case "windows":
                return RESOURCE_DIRECTORY.resolve("lib/java/windows/amd64/pv_leopard_jni.dll").toString();
            case "mac":
            ...
            case "linux":
                return RESOURCE_DIRECTORY.resolve("lib/java")
                        ...
                        .resolve(ARCHITECTURE)
                        .resolve("libpv_leopard_jni.so").toString();
            default:
                return null;
        }
    }
